...
###Paper-level synthesis##################################################################################################           
     ...
     #######INTRODUCTION Statement #################                        
            df$intro <- df %>% 
                        str_glue_data("{Paper_id} A {Exp_design}, {Exp_arrangement} study with {reps_list} replications was conducted at {unique_locations} sites from {Year_start} to {Year_start + Years_num} in {city_state_list} investigating the effects of {Trtmt_list} in a {Cash_species} system ({Authors_abbrev}, {PubYear}).")
unique(df$intro)  
